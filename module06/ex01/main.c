...
** whether to advance to while (!(TWCR & (1 << TWINT)));the next stage of the sequence:
**
...
**
**   AHT20_POLL    g_tick >= 25 (25 × 40ms = 1000ms)
...
void    __vector_11(void)
{
	...
	else if (g_phase == AHT20_CONVERT && g_tick >= 2)  // 2 × 40ms = 80ms
	{
		g_tick = 0;
		g_phase = AHT20_BUSY;
		i2c_load(AHT20, NULL, 0, (uint8_t *)g_aht20_rx, 7, on_aht20_data);
		i2c_start();
	}
...
void	__vector_24(void)
{
	...
	switch (status)
	{
		...
		// Three sub-cases:
		//   a) more bytes to send : load next byte into 
		...
		// Store the byte, then decide ACK/NACK TWDR and continue
		...
		// A data byte was received and ACK was sent to the slave.
		// Store the byte, then decide ACK/NACK 
			{
				i2c_stop();
