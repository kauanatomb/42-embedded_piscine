...
/*
**Buffer for last three measurements of temperature & humidity.
** [0..2]: last 3 raw temperature values (20-bit, not yet converted)
** [3..5]: last 3 raw humidity values    (20-bit, not yet converted)
** Initialised to 0 — the first two readings will average with zeros
** until the ring is full after 3 measurements.
*/
static volatile uint32_t	g_measurements[6] = {0x007FFFF, 0x0007FFFF, 0x0007FFFF, 0x0007FFFF, 0x0007FFFF, 0x0007FFFF};
...
static void on_aht20_data(uint8_t *data, uint8_t len, uint8_t ok)
{
	...
	// Sum the three raw readings, then apply the conversion formula once.
	// Dividing by 3 after the sum preserves full 20-bit precision.
	uint32_t raw_temp = (g_measurements[0] + g_measurements[1] + g_measurements[2]) / 3;
	uint32_t raw_hum  = (g_measurements[3] + g_measurements[4] + g_measurements[5]) / 3;
...
**
**   AHT20_POLL    g_tick >= 25 (50 × 40ms = 2000ms)
...
void    __vector_11(void)
{
	...
	else if (g_phase == AHT20_POLL && g_tick >= 50)    // 50 × 40ms = 2s
	{
		g_tick = 0;
		g_phase = AHT20_BUSY;
